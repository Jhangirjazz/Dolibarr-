...
function llxHeaderVierge($title, $head = "", $disablejs = 0, $disablehead = 0, $arrayofjs = [], $arrayofcss = [])  // @phan-suppress-current-line PhanRedefineFunction
{
	global $conf, $langs, $mysoc;
	   ...
	   // Add reCAPTCHA script if keys are configured
...
if (empty($reshook) && $action == 'add') {	// Test on permission not required here. This is an anonymous form. Check is done on constant to enable and mitigation.
 ...
 // Add reCAPTCHA validation if configured
    ...
    if (!empty($recaptcha_secret) && !empty($recaptcha_sitekey)) {
        $token = GETPOST('g-recaptcha-response', 'none'); // don't strip characters
        
        if (empty($token)) {
            $error++;
            $errmsg .= $langs->trans("ErrorFieldRequired", "reCAPTCHA")."<br>\n";
        } else {
            ...
            if ($resp === false) {
                ...
                if (empty($json['success'])) {
                    $error++;
                    $errmsg .= $langs->trans("InvalidCaptcha")."<br>\n";
                }
            }
        }
    }
...
// Add reCAPTCHA field if configured
$recaptcha_sitekey = getDolGlobalString('PROJECT_RECAPTCHA_SITEKEY');
if (!empty($recaptcha_sitekey)) {
    print '<tr><td colspan="2" style="text-align: center; padding: 20px 0;">';
    print '<div class="g-recaptcha" data-sitekey="'.dol_escape_htmltag($recaptcha_sitekey).'" style="display: inline-block;"></div>';
    print '</td></tr>';
}
print "</table>\n";
