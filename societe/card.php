	...
	if ((!GETPOST('getcustomercode') && !GETPOST('getsuppliercode')) && ($action == 'add' || $action == 'update') && $permissiontoadd) {
		...
		if (!$error) {
			...
			if (GETPOSTINT("private") == 1) {	// Ask to create a contact
				...
				$fullName = trim(GETPOST('firstname','alphanohtml').' '.GETPOST('lastname','alphanohtml'));

					// store it in the extra-field
				$object->array_options['options_full_name'] = $fullName;

				// $object->particulier		= GETPOSTINT("private");
				// // ----- build the "full name" once -----
				// $fullName = trim(GETPOST('firstname','alphanohtml').' '.GETPOST('lastname','alphanohtml'));

				// // 2) Push it into POST so Dolibarr’s helper can see it
				$_POST['options_full_name'] = $fullName; 

				// // $thirdparty->array_options['options_full_name'] = $fullName;     // preferred
				// $_POST['options_full_name'] = $fullName;    // works too

				// $extrafields->setOptionalsFromPost(null, $object, '', 1); 
				// // --------------------------------------
				// // create / update the third-party object
				// // --------------------------------------
				if (GETPOSTINT('private') == 1) {                 // individual card
					$object->particulier = 1;

					$object->name       = $fullName;              // banner
					$object->name_alias = '';                     // no subtitle
					/* … */
				} else {                                          // company card
					$object->name       = GETPOST('name','alphanohtml');  // company banner
					// $object->name_alias = $fullName;                      // person subtitle
				}

								if (GETPOSTISSET('name_alias') && GETPOST('name_alias','alphanohtml') !== '') {
